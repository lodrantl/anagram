var Game = {
	...
	points : 0,
	init : function() {
		console.debug("Session cookie: " + Game.extras.readCookie("AHAHA"));
...
var auth = {
	...
	setNew : function() {
		...
		// @formatter:off
		var content = anagram.sentence.replace(/ _ /g, "<div class='word-name before after'></div>").replace(/ _/g, "<div class='word-name before'></div>").replace(/_ /g,
				"<div class='word-name after'></div>").replace(/_/g, "<div class='word-name'></div>").replace(/%/g, "<span class='separator'></span>");
		// @formatter:on
	...
	addTextHints : function(help) {
		...
		var lastHelp = "Prvi črki imena in priimka:   " + splits[0][0] + Array(splits[0].length).join(" _") + "     " + splits[1][0] + Array(splits[1].length).join(" _");
...
var anag = {
	...
	setNew : function() {
		...
		// @formatter:off
		var content = anagram.sentence.replace(/ _ /g, "<div class='word before after'></div>").replace(/ _/g, "<div class='word before'></div>").replace(/_ /g, "<div class='word after'></div>")
				.replace(/_/g, "<div class='word'></div>").replace(/ \$ /g, "<div class='word-name before after'></div>").replace(/ \$/g, "<div class='word-name before'></div>").replace(/\$ /g,
						"<div class='word-name after'></div>").replace(/\$/g, "<div class='word-name'></div>").replace(/%/g, "<span class='separator'></span>");
		// @formatter:on
...
$(document).ready(Game.validateSession);
